...
<p class="collapse show d-print-none" id="relatedCalcs">
	<?=$ec_lang['ec_related_calcs']?> <a href="Looped-Network.php"><?=$ec_lang['lpn_main_menu']?></a> &middot; <a href="Manning-Irregular.php"><?=$ec_lang['mi_menu']?></a> &middot; <a href="Rock-Chute.php"><?=$ec_lang['rc_main_menu']?></a> &middot; <a href="Manning-Pipe-Flow.php"><?=$ec_lang['mpf_main_menu']?></a> <a data-bs-toggle="collapse" href="#relatedCalcs" aria-expanded="true" aria-controls="relatedCalcs"><?=$ec_lang['view_hide_line']?></a>
</p>
<?php
echoCalculatorForm(
	//Inputs
	Array(
		...
		Array('name' => 'n_in', 'type' => 'number', 'default' => '0.025', 'units' => NULL, 'label' => ecLinkTipLabel('http://www.engineeringtoolbox.com/mannings-roughness-d_799.html', $ec_lang['mpf_manningRoughness'], $ec_lang['mtc_iteration_tip']).'<br /> <input type="radio" name="n_radio" id="n_radio_strickler" value="strickler" onchange="EngCalcs.submitForm()" /><label for="n_radio_strickler">Strickler</label> <input type="radio" name="n_radio" id="n_radio_bb" value="bb" onchange="EngCalcs.submitForm()" /><label for="n_radio_bb">B/B</label> <input type="radio" name="n_radio" id="n_radio_pi" value="pi" onchange="EngCalcs.submitForm()" /><label for="n_radio_pi">P&amp;I</label>'),

...
$ec_lang['lpn_inp_drop_quality']='Цей файл описує, як змінюється якість води в дорозі: що спочатку є у воді, де надходить більше цієї речовини, як вона реагує і як перемішується в ємностях. Ця сторінка обчислює вік води та трасування джерела, але не хімію, тож нічого з цього тут не використовується. Ці рядки зберігаються і будуть записані назад, якщо ви збережете файл EPANET.';
...
$ec_lang['lpn_tip_select']='Використовуйте цей режим, щоб змінювати, пересувати й перетягувати об\'єкти на карті. Сторінка сама повертається до цього режиму після деяких дій, наприклад відкриття проєкту.';
...
$ec_lang['lpn_settings_color_thematic_tip']='Приховує підписи вузлів і з\'єднань, тож на карті лишаються лише кольори. Текст, який ви розмістили самі, залишається. Ваш вибір підписів зберігається, і вимкнення цієї опції поверне їх.';
...
$ec_lang['lpn_file_import_geo']='Відкрити xy-файл на карті…';
$ec_lang['lpn_file_import_geo_tip']='Відкриває файл проєкту або файл EPANET у новій вкладці проєкту на цій сторінці, на карті вулиць. Ви розміщуєте креслення на світі вручну: панорамуйте й масштабуйте до потрібної ділянки, тоді змінюйте розмір і повертайте, щоб підігнати. Якщо x та y у файлі вже є довготою та широтою, натисніть кнопку «Це вже lat/lon», і мережа одразу переміститься туди, куди належить. Кожне введене вами число лишається таким, як є.';
...
$ec_lang['lpn_clean_map_tip']='Прибрати рядок стану й показник координат із карти, щоб знімок екрана показував лише креслення. Ваше креслення не змінюється, і цей вибір не зберігається; перезавантаження сторінки поверне показники.';
...
$ec_lang['lpn_time_no_engine']='Вбудований розв\'язувач обчислює один момент часу за раз, тож це мережа лише на {time}: кожен графік зчитується на цей момент, а кожна ємність досі перебуває на початковому рівні, замість того щоб наповнюватися й спорожнюватися. Підключіться до інтернету один раз, щоб завантажити розв\'язувач EPANET, який розраховує весь період.';
$ec_lang['lpn_time_slider']='Час';
$ec_lang['lpn_time_no_period']='Цей проєкт не має періоду часу, тож показати можна лише один момент. Задайте «Загальний час роботи» в Налаштуваннях, щоб розрахувати мережу за часом.';
...
$ec_lang['lpn_time_run_note']='Ви бачите мережу в перший момент звітування. Розрахунок цієї мережі за весь її період часу триває так довго, що результати для пізніших моментів не оновлюються, поки ви працюєте. Натисніть кнопку «Розрахувати», щоб оновити їх.';
...
$ec_lang['lpn_result_water_age_tip']='Скільки часу вода, що досягла цієї точки, перебуває в мережі. Нормативного обмеження для порівняння немає, тож оцінюйте це число за власною мережею.';
...
$ec_lang['lpn_new_coords']='Координати';
$ec_lang['lpn_new_coords_xy']='xy-координати';
$ec_lang['lpn_new_coords_geo']='lat/lon';
$ec_lang['lpn_new_coords_tip']='Проєкт xy використовує координати, які не є широтою й довготою. Проєкт lat/lon має географічні координати, тож позаду нього можна намалювати карту вулиць або супутниковий знімок, а відмітки вузлів можна прочитати з рельєфу місцевості. Проєкт xy можна перетворити на lat/lon за допомогою «Файл, Відкрити xy-файл на карті».';
$ec_lang['lpn_new_place']='Почати біля цього місця';
$ec_lang['lpn_new_place_tip']='Необов\'язково. Введіть назву міста, вулиці чи орієнтира — і нова карта відкриється саме там. Слова, які ви вводите, надсилаються до служби назв місць OpenStreetMap, яка першого разу запитає вашу згоду. Залиште поле порожнім, і карта відкриється на весь світ.';
$ec_lang['lpn_new_place_hint']='Петалума, Каліфорнія';
$ec_lang['lpn_new_create']='Створити';
$ec_lang['lpn_inp_drop_rules']='Цей файл містить керування на основі правил. Тут вони не застосовуються, тож труби, насоси й клапани, які вони називають, лишаються у стані, записаному у файлі. Самі правила зберігаються і будуть записані назад, якщо ви збережете файл EPANET.';
$ec_lang['lpn_inp_drop_energy']='Цей файл EPANET містить дані для моделювання вартості перекачування. Ця сторінка не обчислює енергію чи вартість, тож ці числа тут ніде не використовуються. Вони зберігаються і будуть записані назад, якщо ви збережете файл EPANET.';
$ec_lang['lpn_inp_drop_tags']='Цей файл присвоює мітки-теги деяким своїм вузлам, трубам чи іншим елементам. На цій сторінці поки немає місця, де можна побачити тег чи змінити його. Теги зберігаються і будуть записані назад, якщо ви збережете файл EPANET.';
$ec_lang['lpn_inp_drop_report']='Цей файл містить власні налаштування EPANET для звітів, які він друкує. Ця сторінка показує свої відповіді по-своєму, тож ці налаштування тут ніде не використовуються. Вони зберігаються і будуть записані назад, якщо ви збережете файл EPANET.';
...
$ec_lang['lpn_inp_drop_quality_options']='Цей файл задає параметри якості води EPANET: вид аналізу якості води та два налаштування, пов\'язані з хімічною речовиною, — відносну дифузійність і допуск якості. Тут обчислюються вік води та трасування джерела, а хімія — ні, тож ці два налаштування зберігаються, але не використовуються. Усі вони будуть записані назад, якщо ви збережете файл EPANET.';
...
$ec_lang['lpn_settings_quality_track_tip']='Виберіть, що розрахунок має відстежувати в трубах: як довго вода перебуває в системі, чи звідки вона надійшла. Жодне з них не потребує концентрації чи швидкості реакції.';
...
$ec_lang['lpn_quality_chemical']='Хімічна речовина, яку називає цей файл (зберігається, не обчислюється)';
$ec_lang['lpn_quality_needs_run']='Якість води переноситься трубами з часом, тож для цього потрібен розв\'язувач EPANET і загальний час розрахунку. Задайте «Загальний час роботи» в розділі «Час», потім натисніть кнопку «Розрахувати».';
...
$ec_lang['lpn_ff_menu']='Витрата на пожежогасіння…';
...
$ec_lang['lpn_ff_title']='Витрата на пожежогасіння';
...
$ec_lang['lpn_ff_required_node_tip']='Витрата на пожежогасіння, потрібна саме в цьому вузлі, за вашим протипожежним нормативом або пожежною службою для типу забудови, яку він обслуговує. Залиште порожнім, і вузол перевірять за числом у полі «Потрібна витрата на пожежогасіння».';
...
$ec_lang['lpn_ff_design_nodes']='Усі інші вузли';

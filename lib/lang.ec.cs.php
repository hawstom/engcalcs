...
$ec_lang['template_translation_help']='Můžete navrhnout lepší formulaci nebo cokoli jiného? Dejte nám prosím vědět.';
...
$ec_lang['ip_worst_case_warn']='Tlak na testovacím emitoru &ge; tlak na přívodu. Toto pravděpodobně není emitor s nejhorším případem, nebo lze potrubí zmenšit.';
$ec_lang['ip_q_ratio']='<span class="ec-help" title="Toto se liší od naší aproximace standardní míry uniformity.">Průtok posledního emitoru &divide; návrhový průtok, q<sub>last</sub>/q<sub>design</sub> <span class="ec-tip">?</span></span>';
...
$ec_lang['ip_notes_3_def']='Modeluje jeden pevný tlak na přívodu (žádná křivka čerpadla), pouze jednu testovací cestu (ne celé pole) a dvouparametrickou křivku emitoru (nastavte exponent blízko 0 pro aproximaci emitoru s kompenzací tlaku). Jsou hlášeny dva různé poměry uniformity, úmyslně oddělené: q<sub>last</sub>/q<sub>avg,field</sub> je aproximace standardní Uniformity distribuce nízké čtvrtiny (průměr nízké skupiny ÷ průměr populace); ale je vypočítána z malého modelovaného vzorku a uživatelsky odhadnuté opravy místo standardního statistického vzorku celého pole. Testovací postranní větev je navíc úmyslně předpokládaný nejhorší případ, takže její surový, neopravený průměr by podhodnocoval skutečný průměr pole a uniformita by vypadala lépe, než ve skutečnosti je; vstup &Delta;tlaku existuje právě proto, aby toto zkreslení vyvážil. Hodnoty uniformity na 1 nebo výše jsou stále možné: znamenají pouze to, že tlak posledního emitoru je na úrovni odhadovaného průměru pole nebo vyšší, takže bod nejnižšího tlaku je u jiného emitoru. To může být proto, že poslední emitor leží v nižší nadmořské výšce, nebo proto, že odhad &Delta;tlaku je příliš malý. q<sub>last</sub>/q<sub>design</sub> je odlišná, ne-uniformitní kontrola oproti jmenovitému průtoku výrobce &mdash; užitečná pro odhalení celkově přetlakovaného nebo nedotlakovaného systému, ale je to samostatná kontrola, kterou je třeba číst spolu s číslem uniformity, protože návrhový/jmenovitý průtok nezávisí na skutečném průměrném provozním tlaku systému.';

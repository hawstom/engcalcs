...
$ec_lang['lpn_empty_hint']='Начните с добавления фонового изображения или резервуара на панели инструментов, либо откройте меню «Файл» → «Новый проект», чтобы начать с примера.';
...
$ec_lang['lpn_v2_restore_prompt']='Этот калькулятор хранит единицы измерения и введённые значения проекта в том виде, в котором они были введены, но раньше числа при сохранении переводились в единицы СИ. Этот проект был сохранён до этого изменения, поэтому его числа хранятся в единицах СИ. Перевести их в текущие единицы в последний раз? Чтобы вы могли оценить результат, вот несколько диаметров, которые будут переведены, с их значениями до и после:';
...
$ec_lang['lpn_new_from_examples']='Из примеров';
$ec_lang['lpn_new_example_us']='Простая сеть, американские единицы (gpm)';
$ec_lang['lpn_new_example_si']='Простая сеть, единицы СИ (л/с)';
...
$ec_lang['lpn_inp_report_counts']='{nodes} узлов и резервуаров, {links} труб и насосов, в единицах {units}.';
...
$ec_lang['lpn_inp_drop_tanks']='Накопительные баки были пропущены. На этой странице есть резервуары, которые поддерживают один фиксированный уровень воды. Накопительный бак не поддерживает фиксированный уровень, поэтому это не резервуар.';
$ec_lang['lpn_inp_drop_tank_links']='Эти трубы были пропущены, поскольку они подключены к баку, который был пропущен.';
$ec_lang['lpn_inp_drop_tcv']='Эти дроссельные регулирующие клапаны перенесены как очень короткие трубы с той же местной потерей. Вода ведёт себя так же, но элемент не тот же самый.';
$ec_lang['lpn_inp_drop_valve']='Эти клапаны регулируют давление или расход, а на этой странице такого элемента нет. Они перенесены как открытые трубы, поэтому сеть остаётся соединённой, но управление ими больше не осуществляется.';
...
$ec_lang['lpn_close_save_prompt']='Сохранить изменения в {name} перед закрытием?';
$ec_lang['lpn_close_browser_prompt']='{name} хранится только в этом браузере. Если закрыть его без сохранения в файл, он будет потерян безвозвратно.';
...
$ec_lang['lpn_file_reconnect_prompt']='Этот проект был получен из {file}. Вашему браузеру снова нужно ваше разрешение, прежде чем он сможет записать в него. Переподключитесь ниже.';
...
$ec_lang['lpn_notes_1_def']='Решает для одного набора расходов отбора за раз, используя тот же алгоритм глобального градиента, что и EPANET. Он не моделирует, как сеть меняется со временем.';
$ec_lang['lpn_notes_2_term']='Не моделируется';
$ec_lang['lpn_notes_2_def']='Баки, качество воды и регулирующие клапаны, которые открываются и закрываются самостоятельно (PRV, PSV, FCV), не моделируются. Труба может иметь фиксированную местную потерю, но не клапан, чьё открытое или закрытое состояние зависит от искомого расхода.';
...
$ec_lang['lpn_diag_no_fixed_head']='Добавьте резервуар. Прежде чем сеть можно будет решить, ей нужен хотя бы один известный уровень воды.';
...
$ec_lang['lpn_backdrop_position_prompt1']='Щёлкните любую точку на фоновом изображении. Это точка, которую вы будете перемещать.';
$ec_lang['lpn_backdrop_position_prompt2']='Выберите, куда должна переместиться эта точка, затем нажмите «Продолжить».';
...
$ec_lang['lpn_settings_engine_epanet']='Решать с помощью ядра EPANET';
$ec_lang['lpn_settings_engine_epanet_tip']='Запускает исходное ядро EPANET Агентства по охране окружающей среды США прямо здесь, в вашем браузере. Встроенный решатель даёт те же ответы и работает быстрее, поэтому оставляйте эту настройку выключенной, если вам не нужен именно EPANET.';
$ec_lang['lpn_engine_loading']='Загрузка ядра EPANET…';
$ec_lang['lpn_engine_failed']='Не удалось загрузить ядро EPANET. Вместо него показан встроенный решатель.';
...
$ec_lang['lpn_settings_text_size']='Размер текста';
$ec_lang['lpn_settings_text_size_map']='Расстояние на карте';
$ec_lang['lpn_settings_text_size_screen']='Пиксели экрана';
$ec_lang['lpn_settings_symbol_size']='Размер символа (относительно текста)';
...
$ec_lang['lpn_settings_text_size_units']='Единицы размера текста';

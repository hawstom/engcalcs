...
$ec_lang['mhp_notes_6_def']='Типичный КПД установки &eta; составляет от 0,70 до 0,85 для турбин Пелтона и поперечноструйных турбин, распространённых в малой гидроэнергетике. Используйте 0,75 в качестве консервативной первоначальной оценки.';
...
$ec_lang['ip_dp_avg']='<span class="ec-help" title="На сколько выше (или ниже) вы считаете, что типичная/средняя латераль работает по сравнению с этой тестовой латералью. Тестовая латераль специально принята за наихудший случай, поэтому её собственное среднее — смещённо-низкий заменитель полевого среднего — при оставлении на 0, проверка однородности и числа из раздела «Проектирование полива» ниже используют собственное (вероятно оптимистичное) среднее тестовой латерали как есть.">Оценённая &Delta;давления, среднее по сравнению с тестовой латералью <span class="ec-tip">?</span></span>';
...
$ec_lang['ip_notes_5_def']='Интенсивность полива и расход системы/зоны используют оценённый средний полевой расход эмиттера (q<sub>avg,field</sub> — собственное среднее тестовой латерали, скорректированное введённой оценкой &Delta;давления), а не предположительную норму: PR = q<sub>avg,field</sub> / A<sub>e</sub>, снабжено скорректированным модельным значением. Расстояние, а также количество латералей и эмиттеров по всей системе — отдельные входные данные здесь потому, что тестовая трасса моделирует только одну наихудшую ветвь, не каждую латераль в поле.';

...
$ec_lang['lpn_inp_drop_quality']='Этот файл описывает, как меняется качество воды при её движении: что изначально содержится в воде, где в неё поступает больше этого вещества, как оно реагирует и как перемешивается в баках. Эта страница вычисляет возраст воды и трассировку источника, но не химию, поэтому здесь ничего из этого не используется. Эти строки сохраняются и записываются обратно, если вы сохраните файл EPANET.';
...
$ec_lang['lpn_tip_select']='Используйте этот режим, чтобы изменять, перемещать и перетаскивать объекты на карте. Страница сама возвращается в этот режим после некоторых действий, например открытия проекта.';
...
$ec_lang['lpn_settings_color_thematic_tip']='Скрывает подписи узлов и связей, оставляя на карте только цвета. Текст, который вы разместили сами, остаётся. Ваш выбор подписей сохраняется, и при отключении этой настройки они появятся снова.';
...
$ec_lang['lpn_file_import_geo']='Открыть xy-файл на карте…';
$ec_lang['lpn_file_import_geo_tip']='Открывает файл проекта или файл EPANET в новой вкладке проекта на этой странице, поверх карты улиц. Вы размещаете чертёж на местности вручную: панорамируете и масштабируете карту к нужному месту, затем меняете размер и поворот, чтобы он подошёл. Если x и y в файле уже являются долготой и широтой, нажмите кнопку «Это уже широта/долгота», и сеть сразу переместится туда, где ей место. Каждое введённое вами число остаётся таким, как есть.';
...
$ec_lang['lpn_clean_map_tip']='Убирает с карты строку состояния и координатную индикацию, чтобы на снимке экрана была видна только карта. Ваш чертёж при этом не меняется, и этот выбор не сохраняется: при перезагрузке страницы индикация вернётся.';
...
$ec_lang['lpn_georef_asdegrees']='Значения x и y в этом файле были прочитаны как долгота и широта, поэтому сеть уже находится на карте и ничего не перемещалось. Проверьте, что она в нужном месте, затем нажмите кнопку «Сохранить это положение».';
...
$ec_lang['lpn_result_water_age_tip']='Как долго вода, дошедшая до этой точки, находится в системе. Нормативного предела для сравнения нет, поэтому оценивайте это число по своей собственной системе.';
...
$ec_lang['lpn_new_coords']='Координаты';
$ec_lang['lpn_new_coords_xy']='координаты xy';
$ec_lang['lpn_new_coords_geo']='широта/долгота';
$ec_lang['lpn_new_coords_tip']='В проекте xy координаты не являются широтой и долготой. В проекте «широта/долгота» координаты географические, поэтому под чертежом можно показать карту улиц или снимок со спутника, а отметки узлов можно считывать с рельефа местности. Проект xy можно преобразовать в «широта/долгота» через «Файл, Открыть xy-файл на карте».';
$ec_lang['lpn_new_place']='Начать рядом с этим местом';
$ec_lang['lpn_new_place_tip']='Необязательно. Введите название города, улицы или ориентира, и новая карта откроется там. Введённый текст отправляется в службу поиска мест OpenStreetMap, которая при первом обращении спросит ваше разрешение. Оставьте поле пустым, и карта откроется на весь мир.';
$ec_lang['lpn_new_place_hint']='Петалума, Калифорния';
$ec_lang['lpn_new_create']='Создать';
$ec_lang['lpn_inp_drop_rules']='В этом файле есть управления на основе правил. Здесь они не применяются, поэтому трубы, насосы и клапаны, которые они называют, остаются в том состоянии, которое записано в файле. Сами правила сохраняются и записываются обратно, если вы сохраните файл EPANET.';
$ec_lang['lpn_inp_drop_energy']='Этот файл EPANET содержит данные для расчёта стоимости перекачки. Эта страница не вычисляет энергию или стоимость, поэтому эти числа здесь не используются. Они сохраняются и записываются обратно, если вы сохраните файл EPANET.';
$ec_lang['lpn_inp_drop_tags']='В этом файле некоторым узлам, трубам или другим элементам присвоены метки. На этой странице пока негде посмотреть метку или изменить её. Метки сохраняются и записываются обратно, если вы сохраните файл EPANET.';
$ec_lang['lpn_inp_drop_report']='В этом файле хранятся собственные настройки EPANET для печатаемых им отчётов. Эта страница показывает свои результаты по-своему, поэтому здесь они не используются. Они сохраняются и записываются обратно, если вы сохраните файл EPANET.';
...
$ec_lang['lpn_inp_drop_quality_options']='В этом файле заданы параметры качества воды EPANET: Quality называет вид анализа качества воды, а два параметра, относящихся к химическому веществу, — Relative diffusivity и Quality tolerance. Здесь вычисляются возраст воды и трассировка источника, а химическое вещество — нет, поэтому эти два параметра сохраняются без использования. Все они записываются обратно, если вы сохраните файл EPANET.';
...
$ec_lang['lpn_settings_quality_track_tip']='Выберите, что должен отслеживать расчёт по трубам: как долго вода находится в системе или откуда она пришла. Ни для того, ни для другого не нужны концентрация или скорость реакции.';
...
$ec_lang['lpn_quality_chemical']='Химическое вещество, названное в этом файле (сохраняется, но не вычисляется)';
...
$ec_lang['lpn_ff_menu']='Противопожарный расход…';
...
$ec_lang['lpn_ff_title']='Противопожарный расход';
...
$ec_lang['lpn_ff_required_node_tip']='Противопожарный расход, требуемый именно в этом узле, по вашим противопожарным нормам или вашей пожарной службе для обслуживаемого им вида застройки. Оставьте поле пустым, и узел будет проверен по числу в поле «Требуемый противопожарный расход».';
...
$ec_lang['lpn_ff_design_nodes']='Все остальные узлы';

...
EngCalcs.clipboardCopy = function() {
	'use strict';
	this.cookieValueToCsv();
	if (this.debugMode) {
		alert(this.cookieValue);
		alert(this.csv);
	} else {
		window.navigator.clipboard.writeText(this.csv);
	}
};

EngCalcs.clipboardPaste = function() {
	'use strict';
	if (this.debugMode) {
		// Round trip conversion test.
		this.cookieValueToCsv();
		this.csvToCookieValue();
		alert(this.cookieValue);
	} else {
		window.navigator.clipboard.readText().then((clipText) => (this.csv = clipText));
		this.csvToCookieValue();
		this.createCookie();
		this.readAndCalc();
	}

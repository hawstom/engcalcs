...
$ec_lang['lpn_field_text_match_pipe']='与最近的管道对齐';
...
$ec_lang['lpn_time_format_tip']='请以小时和分钟的形式输入时间，例如 2:30。单独一个数字表示小时；带有单位词的数字则按该单位理解，例如 30 分钟。';
$ec_lang['lpn_time_running']='正在使用 EPANET 引擎计算整个时间段。';
$ec_lang['lpn_time_no_engine']='内置求解器一次只能算出一个时刻，因此这里显示的仅是 {time} 时刻的管网状态：各需水量采用该时刻的模式乘数，且每个水箱仍处于其起始水位，而非随时间充放水变化。请连接一次互联网以获取 EPANET 引擎，它可以计算整个时间段。';

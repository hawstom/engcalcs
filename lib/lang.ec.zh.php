...
$ec_lang['lpn_result_water_age_tip']='到达该点的水在系统中停留的时长。此项没有可比照的法定限值，请根据自己管网的实际情况判断该数值。';
...
$ec_lang['lpn_new_coords_xy']='xy 坐标';
$ec_lang['lpn_new_coords_geo']='经纬度';
$ec_lang['lpn_new_coords_tip']='xy 项目使用的坐标不是经纬度。经纬度项目具有地理坐标，因此可以在其后绘制街道地图或卫星图片，还可以从地形读取节点高程。可以通过“文件”、“在地图上打开 xy 文件”将 xy 项目转换为经纬度项目。';
$ec_lang['lpn_new_place']='从这个地点附近开始';
$ec_lang['lpn_new_place_tip']='可选。输入城镇、街道或地标名称，新地图会在该处打开。您输入的文字会发送给 OpenStreetMap 的地名服务，该服务首次使用时会请求您的许可。留空则地图会在整个世界范围打开。';
$ec_lang['lpn_new_place_hint']='杭州市西湖区';
$ec_lang['lpn_new_create']='创建';
$ec_lang['lpn_inp_drop_rules']='此文件包含基于规则的控制。这些规则在本页面中不会被执行，因此规则中指定的管道、水泵和阀门会保持文件中写明的状态。规则本身会被保留，如果您保存为 EPANET 文件，会被写回。';
$ec_lang['lpn_inp_drop_energy']='此 EPANET 文件包含水泵运行成本模型数据。本页面不计算能耗或成本，因此这里不会用到这些数字。它们会被保留，如果您保存为 EPANET 文件，会被写回。';
$ec_lang['lpn_inp_drop_tags']='此文件为部分节点、管道或其他元件指定了标签。本页面目前还没有查看或更改标签的地方。标签会被保留，如果您保存为 EPANET 文件，会被写回。';
$ec_lang['lpn_inp_drop_report']='此文件包含 EPANET 自身用于打印报告的设置。本页面以自己的方式显示结果，因此不会用到这些设置。它们会被保留，如果您保存为 EPANET 文件，会被写回。';
$ec_lang['lpn_inp_drop_sections']='此文件包含一个本页面完全不读取的部分。这里不会用到它。它会被完整保留，如果您保存为 EPANET 文件，会被写回。';
$ec_lang['lpn_inp_drop_quality_options']='此文件设置了 EPANET 水质选项：Quality（指定水质分析的种类），以及两项与化学物质相关的设置——相对扩散系数和水质容差。本页面可计算水龄和来源追踪，但不计算化学反应，因此这两项设置会被保留但不会被使用。如果您保存为 EPANET 文件，全部内容都会被写回。';
...
$ec_lang['lpn_settings_quality_track_tip']='选择本次运行要沿管道追踪的内容：水在系统中停留的时长，或水的来源。两者都不需要浓度或反应速率。';
...
$ec_lang['lpn_quality_chemical']='此文件指定的某种化学物质（仅保存，不计算）';
$ec_lang['lpn_quality_needs_run']='水质沿管道随时间传播，因此需要 EPANET 求解器和总运行时长。请先在“时间”下设置总运行时长，然后点击“计算”按钮。';
...
$ec_lang['lpn_ff_required_node_tip']='按您的消防规范或消防部门针对该节点所服务用地性质要求的消防流量。留空则按“所需消防流量”框中的数值进行测试。';
...
$ec_lang['lpn_clean_map_tip']='从地图上去掉状态栏和坐标读数，使截图只显示图纸本身。您的图纸不受影响，且此选择不会被保存；重新加载页面后读数会恢复显示。';
...
$ec_lang['lpn_file_import_geo']='在地图上打开 xy 文件…';
$ec_lang['lpn_file_import_geo_tip']='在本页面新建一个项目标签页中打开一个项目文件或 EPANET 文件，并将其显示在街道地图上。您需要手动将图纸放置到世界地图上：平移和缩放到现场位置，然后调整大小和角度以对齐。如果文件中的 x 和 y 本身就是经度和纬度，请点击“这些已经是经纬度”按钮，管网会立即跳转到其应处的位置。您输入的每个数字都会保持不变。';
...
$ec_lang['lpn_inp_drop_quality']='此文件描述了水质在传输过程中的变化：水中最初含有什么、这些物质从何处进一步进入水中、它们如何反应，以及水箱如何混合。本页面计算水龄和来源追踪，但不计算化学反应，因此这些内容都不会被使用。这些行会被保留，如果您保存为 EPANET 文件，会被写回。';
...
$ec_lang['lpn_settings_color_thematic_tip']='隐藏节点和连接线标签，使地图上只保留颜色。您自行放置的文字仍会保留。您的标签设置会被保留，关闭此选项后即可恢复。';
...
$ec_lang['lpn_time_no_engine']='内置求解器一次只能算出一个时刻，因此这里显示的仅是 {time} 时刻的管网状态：每个模式都按该时刻取值，每个水箱仍处于起始水位，而不会随时间充放水。请连接一次互联网以获取 EPANET 求解器，它可以运行整个时间段。';
$ec_lang['lpn_time_no_period']='此项目没有设置时间段，因此只有一个时刻可以显示。请在“设置”中设置总运行时长，以便按时间计算管网。';
$ec_lang['lpn_time_run_note']='您看到的是首个报告时刻的管网状态。这个管网整个时间段的计算耗时太长，因此在您操作期间不会持续更新后续时刻的结果。请点击“计算”按钮以更新结果。';
...
$ec_lang['lpn_tip_select']='使用此模式可更改、移动和拖动地图上的内容。页面在完成某些操作后（例如打开一个项目）会自动回到此模式。';

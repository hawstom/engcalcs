...
$ec_lang['lpn_file_convert_as']='在地图上打开 xy 文件…';
$ec_lang['lpn_file_convert_as_tip']='在本页面新建一个项目标签页中打开一个项目文件或 EPANET 文件，并将其显示在街道地图上。您需要手动将图纸放置到世界地图上：平移和缩放到现场位置，然后调整大小和角度以对齐。如果文件中的 x 和 y 本身就是经度和纬度，请点击“这些已经是经纬度”按钮，管网会立即跳转到其应处的位置。您输入的每个数字都会保持不变。';

...
$ec_lang['dw_kinematic_viscosity']='运动粘度，ν <span title="1×10⁻⁶ m²/s for clean water at 20°C" class="ec-tip">?</span>';
...
$ec_lang['mtc_side_slope_1']='边坡 1，z1（水平/垂直）';
$ec_lang['mtc_side_slope_2']='边坡 2，z2（水平/垂直）';
$ec_lang['mtc_channel_slope']='渠道坡度，S';
$ec_lang['mtc_flow_depth']='水深，y';
$ec_lang['mtc_bend_angle']='弯道角度，β <a target="_blank" href="riprap-bend-angle.png" title="点击查看图片">?</a>（用于石材护坡设计）';
$ec_lang['mtc_sgrock']='石材比重，SG（2.65）';
...
$ec_lang['mtc_vel_high']='流速偏高 — 检查过渡段损失、可用能量和水锤风险。';
...
$ec_lang['or_diameter']='直径或高度，D <span title="Diameter for circular; height for rectangular" class="ec-tip">?</span>';
$ec_lang['or_width']='宽度，W <span title="Rectangular openings only" class="ec-tip">?</span>';
$ec_lang['or_invert']='底部高程 <span title="Bottom of opening" class="ec-tip">?</span>';
...
$ec_lang['odt_h1_elev']='初始水面高程 <span title="Starting water surface elevation" class="ec-tip">?</span>';
$ec_lang['odt_a1']='初始面积，A<sub>1</sub>';
$ec_lang['odt_h2_elev']='终止水面高程';
$ec_lang['odt_h_orifice']='孔口形心高程';
$ec_lang['odt_a0']='孔口高程处面积，A<sub>0</sub>';
$ec_lang['odt_a_ending']='终止面积，A<sub>2</sub> <span title="Interpolated from conic model at ending elevation" class="ec-tip">?</span>';
$ec_lang['odt_h2_check']='终止高程校验';
$ec_lang['odt_h2_ok']='终止高程高于孔口顶部 ✓';
$ec_lang['odt_h2_warn']='警告：终止高程等于或低于孔口顶部（形心 + D/2）';
$ec_lang['odt_d']='孔口直径 D <span title="Diameter (circular) or height (rectangular)" class="ec-tip">?</span>';
$ec_lang['odt_w']='孔口宽度，W <span title="Rectangular only" class="ec-tip">?</span>';
...
$ec_lang['odt_h1']='初始水头，H<sub>1</sub> <span title="Starting WSE minus orifice centroid elevation" class="ec-tip">?</span>';
...
$ec_lang['ip_se']='灌水器间距，S<sub>e</sub>';
$ec_lang['ip_sl']='支管间距，S<sub>l</sub>';
$ec_lang['ip_n_e']='每条支管灌水器数, n<sub>e</sub>';
$ec_lang['ip_n_l']='每个灌区支管数, n<sub>l</sub>';
$ec_lang['ip_d']='目标灌水深度, d';
$ec_lang['ip_a_e']='每个灌水器控制面积, A<sub>e</sub>';
$ec_lang['ip_pr']='灌水强度，PR';
$ec_lang['ip_q_lat']='每条支管流量, Q<sub>lat</sub>';
...
$ec_lang['cs_Ec_target']='衬砌目标，E<sub>c,target</sub> <span title="衬砌后的输水效率目标；分数 0–1" class="ec-tip">?</span>';
...
$ec_lang['cs_payback_years']='投资回收期 <span title="Simple payback = total lining cost ÷ annual value recovered" class="ec-tip">?</span>';
...
$ec_lang['ip_main_title']='免费在线灌溉压力与分布均匀性计算器';
$ec_lang['ip_main_desc']='测试支管压力与均匀性估算';
$ec_lang['ip_h_supply']='供水压力';
$ec_lang['ip_elev_supply']='供水高程，z<sub>supply</sub>';
$ec_lang['ip_q_design']='灌水器设计流量，q<sub>design</sub>';
$ec_lang['ip_h_design']='灌水器设计压力';
$ec_lang['ip_x']='灌水器流量指数，x <span title="0.5用于标准非补偿灌水器；接近0用于压力补偿灌水器" class="ec-tip">?</span>';
...
$ec_lang['ip_is_lateral']='支管 <span title="选中：该管段是测试支管的一部分，由各灌水器分流。未选中：该管段是干管，仅向不在测试路径上的支管输送流量。" class="ec-tip">?</span>';
$ec_lang['ip_count']='灌水器 <span title="支管行：该管段内的灌水器数。干管行：从该管段分支出来的除本支管外的其他支管上的灌水器总数。对于测试支管自身分支处的管段，这也包括沿干管向下游分支出去的任何支管，或共享同一接点（例如对边支管）——它们的流量也从这同一管段分支。" class="ec-tip">?</span>';
$ec_lang['ip_length']='L';
$ec_lang['ip_diameter']='D';
$ec_lang['ip_e']='e <span title="达西-韦斯巴赫绝对粗糙度" class="ec-tip">?</span>';
$ec_lang['ip_km']='K<sub>m</sub> <span title="该管段管件/阀门/联接器的通用局部损失系数（≥0.5建议最小值）" class="ec-tip">?</span>';
$ec_lang['ip_elev_ds']='下游高程 <span title="该管段下游端高程。对于中间行可选（若留空默认为平坡/与上方节点相同）。对于最后一行是必填项：该值是最后灌水器的高程，直接决定所需供水压力。" class="ec-tip">?</span>';
$ec_lang['ip_elev_ds_missing_warn']='最后灌水器高程（最后一行）留空，默认为平坡——输入该值以获得准确结果⚠';
$ec_lang['ip_flow']='流量';
$ec_lang['ip_press']='压力';
$ec_lang['ip_v']='V';
...
$ec_lang['ip_hl']='H<sub>l</sub> <span title="管段总损失，H_f + H_m" class="ec-tip">?</span>';
$ec_lang['ip_pressure_warn']='低/负压——检查是否存在负压条件';
$ec_lang['ip_pressure_warn_short']='低⚠';
$ec_lang['ip_h_far']='最后灌水器压力';
$ec_lang['ip_q_supply']='测试路径供流，Q<sub>supply</sub> <span title="进入模型化测试路径的流量，不包括整个灌区/系统——见下方应用设计中的Q_zone作为系统总流量。" class="ec-tip">?</span>';
$ec_lang['ip_q_critical']='最后灌水器流量，q<sub>last</sub>';
$ec_lang['ip_q_avg_lateral']='平均灌水器流量（测试支管），q<sub>avg</sub>';
$ec_lang['ip_dp_avg']='估计Δ压力，平均值与测试支管 <span title="你认为典型/平均支管相比该测试支管高低多少。测试支管故意假定为最坏情况，所以其自身平均值是对田间平均值的偏低代用——置为0时，下面的均匀性检查和应用设计数值使用测试支管自身（可能偏乐观）的平均值。" class="ec-tip">?</span>';
$ec_lang['ip_q_avg_field']='估计田间平均灌水器流量，q<sub>avg,field</sub> <span title="q_avg_lateral在每条支管行的压力加上上面输入的压力差条件下重新计算——试图纠正测试支管为最坏情况而非代表性情况的假设。同时供给下面的均匀性检查和应用设计部分。" class="ec-tip">?</span>';
$ec_lang['ip_du_estimate']='均匀性检查，q<sub>last</sub>/q<sub>avg,field</sub> <span title="最后灌水器计算流量除以估计的田间平均灌水器流量——形式上与教科书中的低四分位分布均匀度相同（低分组平均值÷总体平均值），但基于小模型样本和用户估计的修正，而非全田统计样本。值等于或高于1是可能的且不是错误：只是意味着相对于估计的田间平均值，最后灌水器不是最低点（例如有利的下坡段，或上面的Δ压力估计过小）。" class="ec-tip">?</span>';
$ec_lang['ip_worst_case_warn']='测试灌水器压力等于或高于供水压力——该路径不太可能是你的最坏情况支管。检查是否有更远距离或更高摩阻的支管，或更好代表实际最坏情况的高程线。';
$ec_lang['ip_q_ratio']='最后灌水器流量与设计流量，q<sub>last</sub>/q<sub>design</sub> <span title="单独的检查，不是均匀性指标：最后灌水器与制造商额定设计流量的偏离程度。高于1：运行在设计点以上（例如来自有利的下坡高程增益）。低于1：运行在设计点以下（典型的摩阻主导情况）。" class="ec-tip">?</span>';
$ec_lang['ip_no_solution']='无解：所需供水压力超过输入的供水压力。增加供水压力、降低需求，或使用更大管径。';
$ec_lang['ip_notes_1_term']='方法';
$ec_lang['ip_notes_1_def']='对最后（最远）灌水器处的压力进行猜测，然后沿能量坡降线逐个管段向供水端回推，逐段加上摩阻损失和局部损失。在各节点通过反向计算高程和流速水头来报告实际压力。调整猜测的远端压力（二分法），直到计算的所需供水压力与输入的供水压力相匹配——这是与曼宁管流计算器上的管流求解器处理的相同闭环问题，扩展到分支管网。';
$ec_lang['ip_notes_2_term']='干管与支管管段';
$ec_lang['ip_notes_2_def']='每行代表从供水端到最后灌水器的单一水力最坏路径（测试路径）上的一个管段。干管管段只向不在测试路径上的支管输送流量，其流量取值是一个简单乘积（设计流量×该管段灌水器总数）——无局部压力敏感性。干管是共享的主管，所以在测试支管自身分支处的管段必须包括不仅是其两个端点之间的支管，还包括沿干管向下游分支出去的任何支管，或共享同一接点（例如对边支管）——它们的流量在分支前通过同一管段，无论它们是否出现在此表其他地方。支管管段是测试支管本身的一段：灌水器流量通过实际局部压力按q=k·H<sup>x</sup>计算，摩阻损失通过Christiansen的F(n)系数予以削减，以计入该管段内每个灌水器分流时流量逐级下降的情况。';
$ec_lang['ip_notes_3_term']='限制条件';
$ec_lang['ip_notes_3_def']='模型仅考虑一个固定进口压力（无泵曲线）、一条测试路径（非整个田间）和二参数灌水器曲线（将指数设为接近0以近似压力补偿灌水器）。报告两个不同的比值，故意分开：q<sub>last</sub>/q<sub>avg,field</sub>是均匀性检查，形式上如教科书中的低四分位分布均匀度（低分组平均值÷总体平均值），但由测试支管自身模型灌水器计算，通过输入的Δ压力估计予以修正而非全田统计样本——测试支管故意假定为最坏情况，所以其原始未修正平均值会低估真实田间平均值，使均匀性看起来比实际更好；Δ压力输入特别用于抵消该偏差。值等于或高于1仍是可能的（例如Δ压力估计过小，或有利的下坡段）。q<sub>last</sub>/q<sub>design</sub>是不同的、非均匀性检查，针对制造商额定流量——对发现系统总体超压或欠压很有用，但不能替代均匀性指标，因为设计/额定流量与系统实际平均工作压力无必然关联。';
$ec_lang['ip_notes_4_term']='参考文献';
$ec_lang['ip_notes_4_def']='Christiansen, J.E. (1942). "Irrigation by sprinkling." California Agricultural Experiment Station Bulletin 670. ASAE/ASABE微灌设计标准采用相同的多出口摩阻损失方法。';
$ec_lang['ip_notes_5_term']='应用设计';
$ec_lang['ip_notes_5_def']='灌水强度和灌区/系统流量使用估计的田间平均灌水器流量（q<sub>avg,field</sub>——测试支管自身平均值，通过输入的Δ压力估计修正），而非猜测速率：PR=q<sub>avg,field</sub>/A<sub>e</sub>，由修正的模型值供给。间距和系统范围支管/灌水器计数是这里的单独输入，因为测试路径仅模拟一条最坏情况支管，而非田间的每条支管。';

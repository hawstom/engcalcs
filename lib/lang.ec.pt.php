...
$ec_lang['template_translation_help']='Pode sugerir uma redação melhor ou qualquer outra coisa? Conte-nos.';
...
$ec_lang['ip_du_estimate']='<span class="ec-help" title="Vazão calculada do último emissor dividida pela vazão média estimada do emissor no campo &mdash; esta é uma aproximação da Uniformidade de Distribuição do quarto inferior padrão (média do grupo inferior &divide; média da população); é obtida de uma pequena amostra modelada e de uma correção estimada pelo usuário, em vez de uma amostra estatística completa de campo. Valores iguais ou superiores a 1 são possíveis e válidos: significam apenas que a pressão do último emissor está igual ou acima da média estimada de campo, de modo que algum outro emissor é o ponto de menor pressão. Isso pode ocorrer porque o último emissor está em terreno baixo ou porque a estimativa de &Delta;pressão é pequena demais.">Verificação de uniformidade, q<sub>last</sub>/q<sub>avg,field</sub> <span class="ec-tip">?</span></span>';
$ec_lang['ip_worst_case_warn']='Pressão no emissor de teste &ge; pressão de abastecimento. Este provavelmente não é o emissor de pior caso, ou as tubulações poderiam ser menores.';
$ec_lang['ip_q_ratio']='<span class="ec-help" title="Isso é diferente de nossa aproximação da medida de uniformidade padrão.">Vazão do último emissor &divide; vazão de projeto, q<sub>last</sub>/q<sub>design</sub> <span class="ec-tip">?</span></span>';
...
$ec_lang['ip_notes_3_def']='Modela uma pressão de abastecimento fixa (sem curva de bomba), apenas um caminho de teste (não o campo completo), e uma curva de emissor de 2 parâmetros (defina o expoente próximo de 0 para aproximar um emissor com compensação de pressão). Duas razões de uniformidade diferentes são apresentadas, mantidas deliberadamente separadas: q<sub>last</sub>/q<sub>avg,field</sub> é uma aproximação da Uniformidade de Distribuição do quarto inferior padrão (média do grupo inferior &divide; média da população); porém, é obtida de uma pequena amostra modelada e de uma correção estimada pelo usuário, em vez da amostra estatística completa de campo padrão. Além disso, a lateral de teste é deliberadamente o pior caso presumido, de modo que sua média bruta, sem correção, subestimaria a verdadeira média de campo e faria a uniformidade parecer melhor do que realmente é; a entrada &Delta;pressão existe especificamente para compensar esse viés. Valores iguais ou superiores a 1 ainda são possíveis: significam apenas que a pressão do último emissor está igual ou acima da média estimada de campo, de modo que algum outro emissor é o ponto de menor pressão. Isso pode ocorrer porque o último emissor está em terreno baixo ou porque a estimativa de &Delta;pressão é pequena demais. q<sub>last</sub>/q<sub>design</sub> é uma verificação diferente, sem relação com uniformidade, em comparação com a vazão nominal do fabricante &mdash; útil para detectar um sistema com pressão geral acima ou abaixo do previsto, mas é uma verificação separada, a ser lida junto com o número de uniformidade, já que a vazão de projeto/nominal é independente da pressão média real de operação do sistema.';

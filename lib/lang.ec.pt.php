...
$ec_lang['lpn_clean_map_tip']='Remove a faixa de status e a leitura de coordenadas do mapa, para que uma captura de tela mostre só o desenho. Seu desenho não é alterado, e essa escolha não é salva; recarregar a página traz as leituras de volta.';
...
$ec_lang['lpn_file_import_geo_tip']='Abre um arquivo de projeto ou um arquivo EPANET em uma nova aba de projeto nesta página, sobre o mapa de ruas. Você posiciona o desenho no mundo manualmente: desloque e aplique zoom até o local, depois redimensione e gire para ajustar. Se o x e o y do arquivo já forem uma longitude e uma latitude, pressione o botão Estes já são lat/lon e a rede salta direto para onde pertence. Cada número que você digitou permanece como está.';
$ec_lang['lpn_file_import_geo']='Abrir um arquivo xy no mapa…';
...
$ec_lang['lpn_tip_select']='Use este modo para alterar, mover e arrastar coisas no mapa. A página volta a este modo sozinha depois de algumas ações, como abrir um projeto.';
...
$ec_lang['lpn_settings_color_thematic_tip']='Oculta os rótulos de nó e trecho para que só as cores fiquem no mapa. O Texto que você posicionou permanece. Suas escolhas de rótulos são mantidas, e desligar isto as traz de volta.';
...
$ec_lang['lpn_time_run_note']='Você está vendo a rede no primeiro horário de relatório. Esta rede demora tanto para ser calculada em todo o período de tempo que os resultados dos horários posteriores não são mantidos atualizados enquanto você trabalha. Pressione o botão Executar para atualizá-los.';
$ec_lang['lpn_time_no_engine']='O solucionador interno calcula um momento de cada vez, então isto é a rede apenas em {time}: cada padrão é lido nesse momento, e cada tanque ainda está no seu nível inicial, em vez de encher e esvaziar. Conecte-se à internet uma vez para buscar o solucionador EPANET, que calcula o período inteiro.';
...
$ec_lang['lpn_georef_asdegrees']='O x e o y deste arquivo foram lidos como uma longitude e uma latitude, então a rede já está no mapa e nada foi movido. Verifique se ela está no lugar certo, depois pressione Manter este posicionamento.';
...
$ec_lang['lpn_result_water_age_tip']='Há quanto tempo a água que chega a este ponto está no sistema. Não há um limite regulatório para compará-la, então avalie o número em relação ao seu próprio sistema.';
...
$ec_lang['lpn_new_coords_tip']='Um projeto xy usa coordenadas que não são latitude e longitude. Um projeto lat/lon tem coordenadas geográficas, então um mapa de ruas ou uma imagem de satélite pode ser desenhado atrás dele, e as elevações dos nós podem ser lidas do terreno. Um projeto xy pode ser convertido para lat/lon com Arquivo, Abrir um arquivo xy no mapa.';
$ec_lang['lpn_new_place']='Começar perto deste local';
$ec_lang['lpn_new_place_tip']='Opcional. Digite uma cidade, uma rua ou um ponto de referência, e o novo mapa abre nesse lugar. O texto que você digita vai para o serviço de nomes de lugares do OpenStreetMap, que pede sua permissão na primeira vez. Deixe em branco e o mapa abre no mundo inteiro.';
$ec_lang['lpn_new_place_hint']='Petaluma, Califórnia';
$ec_lang['lpn_new_create']='Criar';
$ec_lang['lpn_inp_drop_rules']='Este arquivo tem controles baseados em regras. Eles não são aplicados aqui, então os trechos, bombas e válvulas que citam permanecem no estado escrito no arquivo. As próprias regras são mantidas, e são gravadas de volta se você salvar um arquivo EPANET.';
$ec_lang['lpn_inp_drop_energy']='Este arquivo EPANET inclui dados de modelagem de custo de bombeamento. Esta página não calcula energia nem custo, então nada aqui usa esses números. Eles são mantidos, e são gravados de volta se você salvar um arquivo EPANET.';
$ec_lang['lpn_inp_drop_tags']='Este arquivo atribui etiquetas a algumas de suas junções, trechos ou outros elementos. Ainda não há um lugar nesta página para ver ou alterar uma etiqueta. As etiquetas são mantidas, e são gravadas de volta se você salvar um arquivo EPANET.';
$ec_lang['lpn_inp_drop_report']='Este arquivo contém as configurações do próprio EPANET para os relatórios que ele imprime. Esta página mostra suas respostas à sua própria maneira, então nada aqui as usa. Elas são mantidas, e são gravadas de volta se você salvar um arquivo EPANET.';
...
$ec_lang['lpn_inp_drop_quality_options']='Este arquivo define opções de qualidade da água do EPANET: Quality, que nomeia o tipo de análise de qualidade da água, e duas configurações associadas a um produto químico, Relative diffusivity e Quality tolerance. Idade da água e Rastreamento de origem são calculados aqui; um produto químico não é, então essas duas configurações são mantidas sem serem usadas. Todas elas são gravadas de volta se você salvar um arquivo EPANET.';
...
$ec_lang['lpn_settings_quality_track_tip']='Escolha o que o cálculo deve acompanhar ao longo dos trechos: há quanto tempo a água está no sistema, ou de onde ela veio. Nenhum dos dois precisa de uma concentração ou de uma taxa de reação.';
...
$ec_lang['lpn_quality_chemical']='Um produto químico que este arquivo nomeia (armazenado, não calculado)';
$ec_lang['lpn_quality_needs_run']='A qualidade da água é transportada pelos trechos ao longo do tempo, então precisa do motor EPANET e de uma duração total da simulação. Defina uma Duração total da simulação em Tempo, depois pressione o botão Executar.';
...
$ec_lang['lpn_ff_menu']='Vazão de incêndio…';
...
$ec_lang['lpn_ff_title']='Vazão de incêndio';
...
$ec_lang['lpn_ff_required_node_tip']='A vazão de incêndio exigida nesta junção em particular, conforme seu código de incêndio ou sua autoridade de incêndio para o uso do solo que ela atende. Deixe em branco e a junção é testada contra o número na caixa Vazão de incêndio exigida.';

...
- [ ] R-075 feat/label-gang-search | I am never going to be happy until I can add 12345678 to the node ID prefix without moving or hiding any of the labels shown. Any such moving or hiding is a blatant bug since adding that string **however** causes no conflicts with anything all the way to Japan. May as well not dodge it, hide it, or paper over it. Find out why it's happening and fix the bad rules.
  ...
  - 1210139e (2026-09-22, after master's symbol cap): still TRUE at 4x and 8x on both drawings. Fit view, empty prefix -> 12345678, of labels the short layout drew: Net3-World moves 5 and hides 18, Net3 moves 7 and hides 16; the floor (labels that genuinely had nowhere to stay) is 18 and 19. Open until the fit view is zero or you rule the floor acceptable.
...
- [x] R-137 feat/label-gang-search | Moving to a different test case than that notorious southwest area, let's look at the northwest area with three properties turned on and we are zoomed in closer. [his screenshot: labels for nodes 120 and 25x at A, well away from their nodes; empty ground at B, nearer them] A human would have slid the two labels at A toward B, shortening the leaders without any bad effects. Could our algorithm be smart enough not to be gratuitously distant like this?
  - 3bf1ade7 + 1210139e (2026-09-22): every drawn leader now moves its label to the nearest clear spot along itself, going round until nothing moves. His corner, ID + elevation + demand + pressure: node 251 28.0 -> 19.1 text heights at 2x (still long: a neighbour's reserved room and leader lie along its line), 257 1.4 -> 1.1 at 2x and 1.4 -> 0.9 at 3x, 120 already 0.8/0.5 after master's symbol cap; longest leader at 3x 23.8 -> 4.8. No new contacts, no fewer labels drawn (at 2x one already-crossing pair swaps which is hidden: 179 for 171). dev/lpn-spike/label-slide-harness.js

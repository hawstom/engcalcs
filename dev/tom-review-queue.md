...
**An ID is permanent and never reused.** Next free: R-089.
...
- [x] R-035 feat/tables-spreadsheet | When I type into any cell after any Undo and press Enter, Tab, or Arrow, my entry is reverted, which is bad. -- feat/tables-spreadsheet 23a57157 -- undo() clones a whole new document, so every cell's <input> was still holding the PRE-UNDO object; refillPaneTable() re-points them on every refill. `pane-review-harness.js`
...
- [x] R-082 feat/tables-spreadsheet | **PERRY FOUND A CRASH, not Tom.** Open a node's Properties, then rename that same node in the table below it: the page throws. The table's rename calls the same FUNCTION as Properties but not the same DOOR -- Properties updates the popup's own id before refreshing, the table does not, so the popup re-renders by an id that no longer exists. -- feat/tables-spreadsheet 10f1277e
- [x] R-083 feat/tables-spreadsheet | **PERRY FOUND A SECOND ONE.** Delete on a selection that includes the ID column fires one blocking dialog PER ROW -- three rows, three dialogs; dozens of rows, dozens of dialogs before the page is usable. No data is lost. -- feat/tables-spreadsheet 10f1277e -- the ID column is skipped by name, so one Delete is one question and never forty

## Round of 2026-09-21 -- the tables, after his own testing

- [x] R-084 feat/tables-spreadsheet | Left and right arrows get stuck at selectors **unless** I first skip over them with Ctrl; interesting. -- the caret could not land on a pull-down at all: paneCellFocusable() looked for a box or a button and not a `<select>`, so focus went to the cell around it, which nothing had made focusable -- and the caret ended up outside the table, where the table's own key handler never hears another key. Ctrl escaped it because a Ctrl+arrow runs THROUGH a pull-down and lands on a real box. `pane-review-2-harness.js`
- [x] R-085 feat/tables-spreadsheet | (a) We have a gratuitous space waster at ID where the goto map icon (nice unsolicited touch!) is a line break below the ID number. Put on same line, and possibly in a separate column (extreme left or transcendent left of the table) to keep the paradigm pure. (b) But I should point out that the right-click menu may upstage this feature, and also (c) an interesting possibility that when we right-click on a support column (like From and To) that contains an asset, we can Show on map that asset instead of the row's asset. -- (a) and (c) BUILT. (a) is `white-space: nowrap` on the ID cell, which is what was costing every row of every table a second line. (c) is a `refTo` declaration on From, To, the customer's pipe and its lumped node; every other column still means the row's own asset. **(b) is his own observation and no column was moved** -- the separate ID column is a change to the column model (Home, the selection indices, the clipboard rectangle) and is his to ask for
- [x] R-086 feat/tables-spreadsheet | When I right-click on a cell near the bottom of the screen, the right-click menu goes off the bottom of the screen. Oops. Fix that. -- it FLIPS above the pointer rather than sliding up under it, on both axes, and clamps only where the menu is taller than the window
- [x] R-087 feat/tables-spreadsheet | Whole-cell near vs left/top... my vote is whole cell at top... the next down movement jumps my cursor to the top of the view, which is disorienting. -- he had diagnosed it exactly: there were two rules, one per direction. Going up aligned the cell's top under the heading, going down aligned its bottom to the floor, which leaves a sliver of a row clipped at the top. **One invariant now, both directions: the scroll always comes to rest on a row boundary, so the row under the heading is always a whole row**, and it sets the scroll ABSOLUTELY rather than nudging whatever the browser's own focus-scroll did first, which is what could jump
- [x] R-088 feat/tables-spreadsheet | It might be nice to develop the tab paradigm of the tables list across the top of the bottom pane. Currently, all the non-active tables are undecorated plain text, which doesn't really say 'I'm an inactive tab.' -- a hairline, a light ground and two rounded top corners: the minimum that reads as a card behind the front one, and not a raised button, which would read as a second toolbar

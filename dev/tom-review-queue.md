...
- [x] R-109 feat/tables-spreadsheet | The page loads with the current table blank. I have to switch away and back to see that table. -- a04de77f -- the table was only ever refreshed at the end of a calculation, so with Recalculate off it stayed empty; it is now drawn whenever a project opens. `pane-review-5-harness.js`, browser spec §46
- [x] R-110 feat/tables-spreadsheet | Sometimes the column widths are unreasonable. For example, Pumps.Date installed, width = 1em; Pumps.Pump head curve, width = 2 em (due to selector?); Pump.Price pattern, width = 3em (due to selector?) -- a04de77f + 9595a1e7 -- your guess was right: a pull-down added nothing to its column's width, so four pull-down columns cut off their own choice. They now hold the column open. A drag also started from a 7em guess instead of the drawn width, and a stored width can no longer break a heading word into more than three pieces. Every column of every table on three examples is checked. `pane-column-drag-harness.js`, §46
- [x] R-111 feat/tables-spreadsheet | Switching to Junctions the first time and some subsequent times delayed about 3 seconds or more. This is the worst issue I found. -- a04de77f -- every tab click threw the table away and rebuilt it from nothing. It is now kept and refilled, and a hidden table keeps its layout. Measured in headless Chrome on Net3: switching back to Junctions 279-560 ms before, 17-97 ms after; the first showing of a table is still a build (about 0.3-0.4 s for Pipes). `pane-review-5-harness.js`, §46, `dev/browser-pass/tables-probe.js`
- [x] R-112 feat/tables-spreadsheet | There is a slight calculation error that makes the top border of the top cell row not quite coincide with the bottom border of the headings, and vary in that from about 0px to 3px (maybe 2px), as I arrow up or down. -- a04de77f -- the rows were 26.6 px tall and the browser scrolls in whole pixels, so the boundary drifted up to half a pixel either way on each row. Row heights are now whole pixels; measured gap is 0 px at all 60 steps of an arrow walk, at 100%, 125% and 150% display scaling. §46
- [x] R-113 feat/tables-spreadsheet | Switching tables (tabs) leaves the tab selected instead of the currently highlighted cell. -- a04de77f -- a tab click now puts the keyboard on the table's current cell (the first cell if none yet), so arrows work at once. `pane-review-5-harness.js`, §46
...
- [x] R-115 feat/tables-spreadsheet | If right-click "Show on map" is "Select on map" instead of just "Go to" or "Zoom to" (which is what I expected), then we should label it "Zoom & select". -- a04de77f -- it selects the element and zooms to it, so it now says "Zoom & select" (the pin's tip too; English only). §46

...
| **Enforceable and not yet enforced** — the rows below | 27 | The decoration that could stop being decoration. **Eight landed on 2026-08-28**: rows 1-3 with the survey, then 3b (a defect its own row-1 sibling had been blind to) and rows 6-9 the same day. **Rows 4, 5 and 3d landed on 2026-08-29, and rows 11, 12, 16 and 19 the same day.** Row 12 is the one worth reading of that batch: writing the check is what found two things on visitors' devices that the inventory claiming to be complete did not list. Row 6 is the one worth reading: writing the check is what discovered the rule named a function that does not exist. |
...
| 3c | **A DEAD READER HIDES A DEAD KEY** — `key_hygiene_check.php` reports "rendered by nothing" by counting references, so a string whose only reader is itself uncalled never appears. Task 542 deleted the terrain menu row and left `lpnTerrainMenuLabel()`/`lpnTerrainMenuTip()`, so two strings live in 27 language files, reach no screen, and are invisible to the check | CLAUDE.md § Language Keys, implied | a reachability walk from the page's own entry points, not a reference count | 90 | Yes — reachability through a dynamic dispatch is undecidable in general, so it must stay ADVISORY and list candidates |

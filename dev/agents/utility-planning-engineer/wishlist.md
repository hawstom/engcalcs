...
**Addendum 2026-09-05 — Tom's new sentence weighed, held.** Tom: *"It seems we should implement
all of EPANET"*, offered as the reason to reconsider `[RULES]`. Two things first: **the premise is
wrong** — `[RULES]` is not off the roadmap, it is Task 248.03 at priority 100 ("Next"), the highest
band on the list; only the language and its editor are parked, and the roadmap already ranks it
above every other gap named below. Second, I went looking for the strongest version of the
opposing case rather than re-reading my own prior paragraph, and found one: **CITED**, real
published work pairs EPANET rule-based control with booster-chlorination SITING and SCHEDULING —
e.g. a 2017 ResearchGate paper "Optimal location and scheduling of booster chlorination stations
using EPANET and PSO," a South Central Connecticut Regional Water Authority (Cherry Hill-Brushy
Valley zone) case study, and an OSTI report on chlorine/decontamination booster optimization
(web search 2026-09-05, summaries not primary reads — treat as secondary pending verification).
That is closer to a planning question than time-of-day pump switching is: siting a booster station
and setting its rough dosing strategy is exactly the kind of thing that belongs in a source-of-
supply chapter. **On inspection it does not overturn the answer, and here is why, stated once:**
in every one of those studies, the RULE TEXT is generated by an automated optimizer (PSO or similar)
wrapped around the EPANET toolkit, not hand-authored by an engineer typing `IF ... THEN ...` into a
network editor. The planning engineer's actual deliverable is the siting decision and the target
residual, both of which this suite can already state as design intent without an `[RULES]` editor;
the rule GRAMMAR is a machine's output format for an operations control system, same as it was
argued to be for pump switching, and my seat still would not open the editor even if it existed. **I
hold my position.** What Tom is reaching for, I believe (SPECULATION, mine, re-derive before
relying on it): not literal EPANET completeness, but discomfort at declining a gap solely on "no
user has one yet" when the gap is mechanically cheap and EPANET has already fully specified it —
see the ranking below, where I recommend exactly that distinction (ship cheap carried-and-understood
sections opportunistically; keep the evidence bar for ones that need real design work, like the
rule LANGUAGE).

---

## 2026-09-05 — Ranking the six remaining EPANET-parity gaps, my seat, and the framing question

Tom's question named six items: `[RULES]` (248.03), `[SOURCES]` and `[MIXING]` and `[TAGS]` and
`[REPORT]` (all under 579), and pump efficiency curves (582, being built by another agent —
ranked, not designed). **My order, highest value to a design-and-planning engineer first:**

1. **Pump efficiency curves (582).** Highest, and not close. **OBSERVED**, journal 2026-09-04: a
   pump energy figure is a PER-grade number — O&M cost, life-cycle-cost analysis, the FWPCA
   §602(b)(13) energy-conservation certification. Running that pump at the network's global
   efficiency instead of its own curve makes the kWh, and therefore the dollar figure, wrong by the
   ratio of the two — silently, on a number this project's own note already calls out for feeding a
   report a regulator reads. **Worth to somebody else:** the same number, same reasons — operations
   budgets off the same kWh.

2. **`[SOURCES]` — booster dose at a node.** Second. The roadmap's "commoner of the two by a
   distance" claim holds up under a look: **CITED**, booster chlorination siting/scheduling is a
   well-populated peer-reviewed literature with real utility case studies (see addendum above), and
   it is a genuine source-of-supply / disinfection-strategy master-plan question — does a new
   pressure zone or a long transmission main need a booster station to hold residual, and roughly
   where. **OBSERVED**, roadmap 579: it is "four fields on a node," EPANET-understood, cheap. My
   seat's case for building it does not need a specific waiting user the way `[RULES]`'s does,
   because the interface cost is near zero and the design question it answers is real and common.
   **Worth to somebody else:** high — this is squarely an operations/water-quality-compliance want
   too, maybe the single most operationally relevant item on this list.

3. **`[TAGS]` — a free-text tag per element.** Third, and this is a **cheap-and-do-it** row, not a
   **wanted-for-its-own-sake** row — I would not have found it myself. **OBSERVED**, roadmap 579:
   natural home for Task 247's customer/account linkage, which IS a planning-adjacent want (rate
   studies, service-area rosters). Small value to ME directly; real value as connective tissue for
   something else already wanted. **Worth to somebody else:** probably highest here for the GIS/
   asset-management side — a tag is exactly the field a utility's GIS shop uses to join an EPANET
   model to its own asset database, and I have no direct evidence for that beyond ordinary GIS
   practice — flagging as **SPECULATION**, re-derive before relying on it.

4. **`[MIXING]` — a tank mixing model.** Fourth, and I would rank it below `[SOURCES]` for a
   specific reason the roadmap doesn't state: it is a refinement on a foundation this suite has not
   built yet. My own wishlist item 4 (water quality, above) is not designed — age, source trace and
   chlorine decay are all still proposals. Building a two-compartment or FIFO tank mixing model
   before there is any reaction/age engine to feed it is finishing a room in a house whose
   foundation is unpoured. **CITED**, when it does matter it matters for real (EPA TCR whitepaper
   and the Eureka, MO case cited in my wishlist item 4a name tank mixing/short-circuiting as a
   genuine cause of a real utility's water-age complaint) — but that is a downstream refinement, not
   a next step. **Worth to somebody else:** water-quality/compliance staff, once age or chlorine
   modeling exists at all; near-zero value to anyone before that.

5. **`[RULES]` — the language and its editor (248.03).** Fifth from THIS seat, even though the
   roadmap already carries it at the top band (100, "Next") — that is a disagreement worth stating
   plainly rather than papering over. My reasoning is unchanged and restated in the addendum above:
   a rule's actual authorship, in the literature I can find, is either a SCADA integrator's fixed
   time-of-day/tank-level logic or an automated optimizer's output, not a design-and-planning
   engineer's own deliverable. **Worth to somebody else: real and specific** — this is exactly the
   ops/SCADA seat's want, which is why the roadmap ranks it where it does; Tom's own priority band
   is presumably weighing that seat, or the file-fidelity argument (a rule's numbers silently wrong
   in the wrong units, already carried in 248.03's own text), which I do not dispute — I am only
   saying my own seat's vote on it, independently, would be lower.

6. **`[REPORT]` — formatting for a `.rpt` this page never requests.** Lowest, and I agree with the
   roadmap's "probably correct to leave carried for ever" — deliberately, not by default. **CITED**,
   web search 2026-09-05 (openepanet.org forum threads, the `epanetReader` R package's own stated
   purpose, general EPANET documentation): real professional use of a raw `.rpt` text file is a
   SCRIPTING/automation pattern — piping EPANET output into Python or R for post-processing,
   calibration loops, or batch analysis — precisely because, in one forum's own words, the report is
   "programmatically generated and not very human friendly." That is a different tool category from
   an interactive map GUI: a GUI's job is to show the same numbers in its own results table, which
   this page already does. Formatting a `.rpt` file would serve a workflow (headless scripted batch
   runs against a `.inp`/`.rpt` pipeline) this suite does not offer and, on my own seat's brief, has
   no reason to start offering — a design-and-planning engineer doing that kind of batch work already
   owns EPANET's own console binary or a Python toolkit for it. **Worth to somebody else:** low here
   too, for the same reason — it would matter to whoever eventually wants scriptable batch analysis
   against this suite, which is a different feature entirely (an API/export surface) and not what
   `[REPORT]`-formatting alone would buy.

**On Tom's framing question directly, no hedge: "implement all of EPANET" is the wrong goal for
this suite, and I hold that from the same evidence this repo already settled on for a different
reason.** **OBSERVED**, `dev/positioning.md:87` and its surrounding section: *"never write a
completeness claim against EPANET... [EPANET] appears to me to be of 'infinite' depth"* (Tom,
2026-08-22, quoted there) — that ruling was about public claims, but the same fact is true of the
engineering choice underneath it. A depth nobody has measured cannot be a target; it can only be a
direction to walk, forever, at whatever pace resources allow. Parity chases whatever gap was
tripped over most recently, which is a different filter from the one this seat is supposed to run
every gap through — **does this change a master-plan or CIP decision at 2,000 nodes** — and the two
filters disagree on this exact list: `[REPORT]` is a total EPANET feature and scores at the bottom;
`[SOURCES]` is four fields and scores near the top. **What I think Tom is actually reaching for**
(SPECULATION, mine): not literal completeness, but frustration that the same sentence — *"why
aren't we implementing this instead of shipping without it"* — keeps recurring (582, 579, and now
this one), and a suspicion that the standing "wait for a user who has one" bar is being invoked as
a blanket brake rather than case-by-case. I think that suspicion is partly right and partly wrong:
right for `[SOURCES]` and `[TAGS]`, where the bar should not apply at all because the marginal cost
is near zero and EPANET has already done the hard part of specifying the feature; wrong for
`[RULES]`, where the bar is protecting something real — a rule's numbers are in the file's own
units and converting them wrong is silent and dangerous, and no amount of "EPANET already specifies
this" changes that the LANGUAGE still has to be written and validated by us. **My actual
recommendation, restated as a policy rather than a one-off:** replace "implement all of EPANET"
with "close a carried-but-unused EPANET section whenever EPANET has already fully specified the
mechanics and the marginal engineering cost is small, independent of a named user; keep the
evidence bar for anything that requires new design work of our own" — which, applied to this exact
list, promotes `[SOURCES]` and `[TAGS]` and leaves `[RULES]`'s language and `[MIXING]`'s model

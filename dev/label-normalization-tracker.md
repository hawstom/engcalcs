...
| 3.4 | Slope naming + S₀ convention | "friction slope" canonical over mpf "pressure slope"; S₀=bed, S_f=friction (mpf currently inverts) — fix to standard | ☑ (Tom, 2026-07-07: standardize to S_f, accept saved-URL break). mpf label → "Friction slope … S<sub>f</sub>"; `mpf_solve_desc` S₀→S_f; Manning-Pipe-Flow slope **field `s0`→`sf`** (page + js/manning-pipe-flow.js, incl. solve fn). Link `pressureslope.php`→`frictionslope.php` (**Tom to rename the external file**; he has already edited its contents to say "friction slope"). Manning-Trap/Irregular keep `s0` (their slope is `mtc_channel_slope`, "Channel slope, S" — bed slope, correct). |
| 3.5 | Flow/velocity/roughness drift | mhp "Flow rate"→"Flow", "Flow velocity"→"Velocity"; roughness `e`→D8 (symbol link + shared tip, see resolved #3); Manning n: `mpf_manningRoughness` canonical, `rc_n_chute` variant (◇) | ☑ flow/vel merged; roughness `e`→D8 done (new `dw_roughness_tip`). **Manning-n D8**: tracker row marks it ◇/canonical, so the invasive symbol-only D8 rewrite (resolved #3 prose) was **deferred** — `mpf_manningRoughness` kept as canonical full label. **Superseded by item 86** (f87a7de): the bare-`e` label was reversed — `dw_roughness`='Roughness, e' for wide forms (dw/mhp), new `ip_roughness`='e' for the narrow column, both still sharing `dw_roughness_tip`. |

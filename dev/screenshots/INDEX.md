...
>
> **AND A THIRD GLYPH GENERATION LANDED 2026-09-04: the tower's tank became an ELLIPTICAL BULB**
> (Tom's own sketch, his "Aft. 1": *"Maybe there is not enough space for double linework"*). So the
> peaked-roof tower is now the OLD mark, exactly as the drop was before it, and **every frame in this
> drop — including the three published on 2026-09-01 — shows a Water glyph that is one generation
> behind.** It is a menu icon a few pixels across and it dates nothing else about a frame, so no row
...
| 0028 | Desktop, English, **no dialog**: Net3 on the Novato street map, nodes coloured by pressure and links by velocity, the colour keys top-RIGHT, the labels legend top-LEFT, and the bottom pane open on **Profile** — ground surface, hydraulic grade line, pressure band | **Yes — the strongest frame we have** | The current file is the 2026-08-29 re-shoot, read from the pixels 2026-09-01: menu bar File / Edit / Map / Water / Help, status strip latitude first, nothing printing through anything, privacy-clean. Two earlier attempts failed on the legends printing over each other; Task 552 fixed the code that let them collide at all. **PUBLISHED 2026-09-01** — `img/0028.png`, md5 `ac5e67fa…`, commit `10eda9e`. |
...
| 0043 | Phone, portrait: Find and replace filling the screen, the query line reading `Junction.ID 5 highest`, then "Change what was found" and the hit list | **Yes** | The current file is the 2026-08-29 re-shoot: Property and Condition share one line and the title sits in the drag band beside the X, both Task 549. The hit list is cut by the bottom edge, and that question is answered by Tom and not by the pixels — *"0043 shows all that would fit."* **PUBLISHED 2026-09-01** — `img/0043.png`, md5 `3abcc37f…`, commit `10eda9e`. |
...
| 0047 | Phone, portrait: the **Water** menu open directly beneath the button that opened it — Settings, Libraries, Profile, Tables, Scenarios, Calculate, EPANET run report — over the network at street level on satellite | **Yes** | The current file is the 2026-08-29 re-shoot. The status strip reads `Latitude: --  Longitude: --`: latitude first, and the `--` is CORRECT rather than a defect, because a phone has no hover and nothing has been pointed at (Task 550). The menu hanging from its own button is Task 528's proof. One cosmetic note, not a blocker: the colour keys print over map content bottom right, and on a phone there is nowhere else for them to go. **PUBLISHED 2026-09-01** — `img/0047.png`, md5 `4ada4ec9…`, commit `10eda9e`. |
...
|---|---|---|
| ~~A phone, in tall mode, with the WATER TOWER in the toolbar~~ **DONE 2026-09-01** | 0043 and 0047 were re-shot on 2026-08-29 and published on Tom's word; both carry the peaked tower, latitude first, and the Task 549 fixes. The glyph moved again on 2026-09-04 (the elliptical bulb) and he has already ruled that a glyph alone does not earn a re-shoot | -- |
...
| ~~A wide desktop frame with today's icons~~ **DONE 2026-09-01** | 0028's 2026-08-29 re-shoot separated the two legends — colour keys top-right, labels legend top-left — and Task 552 fixed the code that let them collide at all. Published on Tom's word | -- |
...
| 0082 | **The everything-at-once frame.** One window, Net3 on the Novato street map, with Settings (searched, Visualization open), the Libraries box, Fire flow analysis, the Pump energy report, Scenario comparison, the Tables pane and a hydraulic Profile all open together | **Yes, and PUBLISHED 2026-09-12** -- `img/0082.png`, md5 `2e7491ee`, on the LibreWaterNet front page as the lead frame and as Plate 1 of screenshots.html | Tom, 2026-09-12: *"This quintessence of what we've built is still not implemented after multiple reminders from me. Do it. Feature it prominently."* Privacy-clean: no browser chrome, no file path, no consent banner, project names `Project1` and `Net3-Novato-CA-World`, which name no client. **RE-SHOT BY TOM 2026-09-18 AND REPUBLISHED THE SAME DAY**, which retired both of the stalenesses this row used to carry: the frame had the EngCalcs navbar that the divorce of 2026-09-11 deleted (Task 625), and a Libraries box predating its Fittings section, which he noticed himself. The picture is no longer older than the software. 1921x920, and `index.html` and `screenshots.html` were corrected to declare that rather than the old 1920x921 -- a declared size that disagrees with the file makes the browser reserve the wrong space and the page jump as it loads |

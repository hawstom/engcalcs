...
| 0082 | **The everything-at-once frame.** One window, Net3 on the Novato street map, with Settings (searched, Visualization open), the Libraries box, Fire flow analysis, the Pump energy report, Scenario comparison, the Tables pane and a hydraulic Profile all open together | **Yes, and PUBLISHED 2026-09-12** -- `img/0082.png`, md5 `684dde32`, on the LibreWaterNet front page as the lead frame and as Plate 1 of screenshots.html | Tom, 2026-09-12: *"This quintessence of what we've built is still not implemented after multiple reminders from me. Do it. Feature it prominently."* Privacy-clean: no browser chrome, no file path, no consent banner, project names `Project1` and `Net3-Novato-CA-World`, which name no client. **TWO KNOWN STALENESSES, published anyway on his instruction and both fixed by one re-shoot** (Task 634): it carries the retired EngCalcs navbar, deleted by the divorce of 2026-09-11 (Task 625), and its Libraries box predates the Fittings section, which Tom noticed himself. Neither is visible as an error to a reader who has not used the page; both make the picture older than the software |

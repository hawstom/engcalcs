...
| 0028 | Desktop, English, **no dialog**: Net3 georeferenced on the Novato street map, nodes coloured by pressure and links by velocity, and the bottom pane open on **Profile** — ground surface, hydraulic grade line, pressure band, "Nodes: 26, length: 79161 ft" | **Yes — the strongest new hero, and the only frame in the drop with today's icons** | **RE-SHOT INTO ITS OWN ORDINAL 2026-08-25, and this row describes the new file** (md5 `e652d7ad…`; the copy published as `img/0028.png` in the librewaternet.org repository is the OLD one, md5 `f879f1f8…`). The new frame is current where the published one is dated twice over: menu bar reads **Water** with the water-tower glyph, not Project with the rolled-plan glyph, and the status strip reads `Latitude: 38.080329 Longitude: -122.709478`, not longitude-first. It answers the remake queue's wide-desktop row. **One thing to fix before it is published:** the Pressure key, the Velocity key and the node/link labels legend are stacked in the top-right corner and the legend is unreadable behind them — that is a PLACEMENT saved in Tom's own `Net3-Novato-CA-World.lwn`, not a defect; today's pointer defaults put the colour key bottom-right and the labels legend top-right, and Task 529 added **Off** |
...
| 0043 | Phone, portrait, **shot on today's master**: Find and replace filling the screen -- What to search: Pipe, Property: Diameter, Condition: equal to, Value 30, the Find and Replace buttons, then "13 found. Click one to go to it." and the hit list beneath | **Not as is — published, and dated by its toolbar** | **Published** on `screenshots.html`, and still the cleanest phone Find-and-replace we have: the panel says "Find and replace" rather than 0001's old "Find assets", and there is no overlay pile because the panel owns the screen. **What has moved under it is the Water glyph.** It was captured at 03:51 on 2026-08-25, inside the two hours the WATER DROP existed; the drop was replaced by a water tower at 04:13 the same day because a drop reads as a tear. The frame is privacy-clean and correct in every other respect, so this is a re-shoot, not a withdrawal |
...
| 0047 | Phone, portrait, on today's master: the **Water** menu open directly beneath the button that opened it -- Settings, Libraries, Profile, Tables, Scenarios, Calculate, EPANET run report -- over the network at street level on satellite, the Flow key at the right, and the units, friction method and scenario stated along the bottom | **Not as is — published, and dated by its toolbar** | **Published** on `screenshots.html`, and still the proof frame for Task 528: the menu hangs from its own button instead of floating in mid-canvas, at a size a thumb can hit. **Same date problem as 0043** — the toolbar shows the water DROP, which lived from 02:13 to 04:13 on 2026-08-25 and was replaced by the water tower. Everything else in the frame is current and privacy-clean. 0046's top-left overlap is behind the menu here and barely reads |
...
| ~~A wide desktop frame with today's icons~~ **SHOT 2026-08-25, not yet published** | 0028 is the re-shoot: the water tower in the menu bar, a latitude-first readout, map above and profile below. What is still on `screenshots.html` is the OLD 0028, which shows the Project menu and a longitude-first readout | The same frame with the two colour keys and the labels legend moved apart or set **Off**, so the top-right corner reads. Then it replaces `img/0028.png` |

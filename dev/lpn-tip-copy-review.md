...
| `lpn_georef_scale_tip` | 43 → 42 | **Before:** …so set it here — or let Go to… ask you how wide the site is and work it out.<br>**After:** …so set it here, or let Go to… ask you how wide the site is and work it out. | Em dash only. See §4. |
| `lpn_menu_project_tip` | 24 → 15 | **Before:** Everything about water network modeling is here in one place, except the animation play controls. There is no need to guess where things are.<br>**After:** Everything about water network modeling is here in one place, except the animation play controls. | The second sentence carried no fact at all, and cost 26 translations of it. |
...
|---|---|---|
| `lpn_library_rule_tip` | 124 | Longest string on the page and entirely EPANET grammar, keyword by keyword. Also **ruled by Tom** on its exact current text. |
| `lpn_file_import_geo_tip` | 81 | Wordy, but the placement instructions are a procedure and the control name has to appear. See §3 for the one thing in it that looks wrong. |
| `lpn_settings_engine_native_tip` | 75 | Long, but every sentence is a fact a reader acts on: which engine solves, what the box does, the three cases that override it, and the 650 KB download. |
| `lpn_labels_priority_node_tip` | 72 | The closing tie-break sentence is genuinely hard reading, but it is the only statement of the rule, and it shares one JS fallback literal with its sibling `lpn_labels_priority_link_tip` in a file this pass may not touch. |
| `lpn_reaction_order_bulk_tip` | 72 | Trimming reached 63 words, and a 9-word gain does not buy 26 retranslations. The closing warning (changing the order moves every answer with no coefficient edited) is the content. |
| `lpn_library_control_tip` | 69 | Four literal EPANET control sentences. The examples are the tip. |
| `lpn_library_curves_note` | 65 | Tom's own current wording, including *indicates* (CLAUDE.md, 2026-09-06). Not ours to shorten. |
| `lpn_settings_demand_multiplier_tip` | 63 | Best available trim was 5 words. |
| `lpn_field_valve_type_tip` | 57 | Best available trim was 4 words, and the closing "because" is what stops a reader reading the reset setting as a defect. |
| `lpn_new_coords_tip` | 55 | Four sentences, four distinct facts, one of them a menu path. |
| `lpn_georef_twopt_tip` | 53 | A four-step procedure; no step is removable. |
| `lpn_library_curve_equation_tip` | 47 | Trim reached 40. The reader does not stumble, and "worked out afresh, never stored" tells them not to look for it in the file. |
| `lpn_field_tag_tip` | 46 | Tom edited this personally on 2026-09-06, striking the EPANET round-trip sentence and keeping the space refusal. |

...
- **Manning Trap Searcy coefficient 0.022: RESOLVED 2026-08-14 — it is natively SI, and the code is correct.** Witheridge, *Background to Rock Sizing Equations* (Catchments & Creeks), Eq 6, states Searcy (1967) as `D50 = 0.022 V^2` in SI; Tom read the source and confirmed. A code review had argued the opposite — that 0.022 is US-customary applied to an SI velocity — on the strength of it matching the Isbash form at K = 0.86 in ft units to within 2%. **That match is a coincidence, not a derivation**, and it is the trap to remember here: read as SI, Searcy sits sensibly below Maynord and both Isbash values (7.8 in against 10.2 / 13.5 / 26.2 at V = 3 m/s, y = 1 m); read as US it lands precisely on one of them, which is the less likely arrangement for two independent authors. Pinned by an assertion in `dev/calc-spike/mtc-harness.js`. Do not change it without Searcy 1967 itself.

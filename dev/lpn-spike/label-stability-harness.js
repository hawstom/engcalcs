...
//
// So this asserts the layout and never the count: the label SET, where every box sits, and whether
// each one drew a leader (label-crossing-measure.js's signature()). Passes after the first must be
// byte-identical to the first.
//
// **ONE EXAMPLE PER PROCESS.** shedAlignedForConflicts() seeds each pass from the LAST layout, so
// two loads in one process contaminate each other (Task 436) -- and that coupling is the mechanism
// of the very defect measured here, which makes the isolation load-bearing rather than tidy.
...
async function main() {
	...
	const modes = process.argv.slice(2).filter(function (a) {
		return /^(off|brute|gang|both)$/.test(a);
	});
	console.log('--- five passes over one untouched view; the layout must not move ---');
	files.forEach(function (f) { (modes.length ? modes : ['both']).forEach(function (m) { check(f, m); }); });

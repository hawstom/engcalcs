...
// (latitude positions, Mercator drawing y) exactly as the seam does in memory. An ELEVENTH is a site nobody wrote a reason for, and a site that should have projected and
// did not is wrong only in a geographic project and only away from the equator.
ok('mercY/mercLat are called from ten places in js/looped-network.js and no more',
	count(/Geom\.mercY\(|Geom\.mercLat\(/g) === 10, count(/Geom\.mercY\(|Geom\.mercLat\(/g));

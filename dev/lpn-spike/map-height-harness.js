...
console.log('\n--- resizing the canvas keeps the view centre, and tiny changes are ignored ---');
{
	...
	function run(env) {
		...
		var fn = new Function('window', 'document', 'svg', 'LPN_MAP_MIN', 'state', 'setTransform', 'pageSettled', 'LPN_MAP_HEIGHT_DEADBAND', 'lastMapBox', 'noteMapSized', 'placeLegends', 'noteMapUnmeasurable', 'mapgeoPlaceDial', 'settings', 'labelWidthLimitSI', 'labelThresholdChanged',
			extract('viewportHeight') + '\n' +
			extract('flowBelowMap') + '\n' + extract('effectiveMapHeight') + '\n' +
		...
		fn({ innerHeight: env.vh, pageYOffset: 0 },
			...
			// The labeling threshold and the symbol cap it sets (Task 705): none is typed in here, so
			// a width change asks nothing more. dev/lpn-spike/zoom-symbol-cap-harness.js drives the
			// resize with one typed.
			{}, function () { return 0; }, function () { /* labelThresholdChanged */ });
...
// **THE PAGE-TITLES SECTION WAS DELETED WITH THE FEATURE** (Task 625, 2026-09-10). It asserted
// that setPageTitlesShown() re-measured the canvas after hiding the three headings -- Tom's
// 2026-08-23 gap. The app page emits no headings at any width now and the toggle is gone, so
// there is nothing left to re-measure. The general rule it protected SURVIVES above and below:
// anything that changes the page's own layout must go through applyMapHeight(), and a window
// resize is still asserted to do so.

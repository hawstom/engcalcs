// **R-208: PROJECT1 OPENS GEOGRAPHIC, AT DOWNTOWN NOVATO CENTER, WITH THE MAP OFF.**
//
...
//
// **A NEW UNREQUESTED THIRD-PARTY REQUEST IS THE OTHER THING THIS HAS TO REFUSE.** A geographic
// project defaults its basemap ON (`basemapOn()` is `project.basemap !== 'off'`, so an UNSET value
// already shows tiles) -- true for every door that reaches "geographic" today, and every one of
// those doors is the visitor's own explicit action (the New Project wizard, Import XY to lat/lon,
// Map > World map > Attach). A first visit is not an action, and `privacy.php` / CLAUDE.md both
// promise the OpenStreetMap tile fetch "asks you first" -- so this ONE project sets
// `project.basemap = 'off'` explicitly, the one geographic project on the whole page that does not
// paint tiles on its own.
...
console.log('--- a first-ever visit: Project1, cold, canvas still unsized ---');
{
	...
	check(!L.basemapOn(), 'the basemap is OFF -- no tile fetch with nobody having asked for one',
	...
	const before = L.basemapOn();
	L.worldMapAttach();
	check(!before && L.basemapOn(), 'Attach turns the map ON, on the visitor\'s own explicit action',
		'before=' + before + ' after=' + L.basemapOn());

...
**124 still to read on master**, of 196 untranslated keys, of 2054 English keys. **A branch section follows if anything is waiting there.** A key already marked _Ruled OK_ below needs nothing from you;
...
## lpn_  (196, 124 to read @@ NEEDS RULING)
...
- **`lpn_crs_unplaceable`**
  ...
  Please explain to me what this is in response to. What's the case for this tip?
- **`lpn_crs_unplaceable_mark`**
  > (no map)
  Please explain to me what this is in response to. What's the case for this label?
...
- **`lpn_field_meter_demand_tip`**
  ...
  Edited. You make the pedantic point in multiple locations that blank is not the same as zero, but this is not a helpful thing to say; it's a truth without information. Instead, say what blank would be used for. I am making an attempt, but I want you to edit my wording if I misunderstood. Please make this a general point learned once we achieve mutual understanding about this.
...
- **`lpn_field_meter_pattern_tip`**
  ...
  Edited. Note that I removed your habitual "and it" construction where you mean "If this, then that."
...
- **`lpn_field_meter_pipe_tip`**
  ...
  This is stale. Connection point can no longer be dragged. Edited. Check.
...
- **`lpn_field_text_all_zoom`**
  ...
  Default needs to be off. I see that you turned it on by default, and that already has caused me grief. Only a few texts would be privileged to have this on. Otherwise ok.
...
- **`lpn_settings_label_max_width_tip`**
  ...
  Should we offer the same hint here as for customer labels (0 to hide always) and remove the Thematic map toggle? Otherwise okay.

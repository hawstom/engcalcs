...
- **`lpn_inp_export_flat_coords`**
  ...
  This is unclear. Please proofread and reword.
...
- **`lpn_customer_fixed_head`**
  ...
  Can you clarify the case? Like a reservoir?
...
- **`lpn_field_meter_lumped_tip`**
  ...
  Edited. This is strictly for accounting, not necessarily physical.
- **`lpn_field_meter_pipe`**
  > Connected asset
  Edited. We want to allow connecting directly to a node.
- **`lpn_field_meter_pipe_tip`**
  ...
  We need to rule on whether we allow non-perpendicular connections.
- **`lpn_field_meter_station`**
  > Station along the pipe (%)
  OK. We need to rule on whether we allow non-perpendicular connections.
...
- **`lpn_tool_add_meter_tip`**
  ...
  See comment for lpn_mode_add_meter
...
- **`lpn_library_import_curve_shape`**
  ...
  Can you clarify "first column rises from each point to the next"?
...
- **`lpn_library_import_units`**
  ...
  This is one situation where we may need to offer conversion.
...
- **`lpn_survey_btn_tip`**
  ...
  I disagree with "has to be". See my prompt comments.
...
- **`lpn_survey_elev_unit`**
  ...
  Under what condition would we know the units of the file?
- **`lpn_survey_err_ambiguous_lat`**
  ...
  Needs discussion and design concept planning.
...
### feat/time-series-graph (`02c6f319`) — 16 new, 16 to read OK.
...
### feat/xy-world-map (`639ea936`) — 9 new, 9 to read OK.
...
- **`lpn_map_attach_turn`**
  ...
  Edited. Changed meaning. Please review carefully.

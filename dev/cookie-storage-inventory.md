...
| `lpn_panecols` | same | How wide the reader dragged each table column, what order they put the columns in, and which columns they hid, per table (Task 690; Tom's spreadsheet specification, 2026-09-18, points (d) and (e); column hide is Declan's own named fix, dev/agents/data-entry-clerk/journal.md). Window furniture by the Task 584 rule: a column's width, order and visibility are facts about the screen somebody is sitting at, so a colleague opening the same project on a laptop must not inherit them. Holds no network data of any kind, only column keys, numbers and booleans |

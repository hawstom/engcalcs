...
| `lpn_panecols` | same | How wide the reader dragged each table column and what order they put the columns in, per table (Task 690; Tom's spreadsheet specification, 2026-09-18, points (d) and (e)). Window furniture by the Task 584 rule: a column width is a fact about the screen somebody is sitting at, so a colleague opening the same project on a laptop must not inherit it. Holds no network data of any kind, only column keys and numbers |

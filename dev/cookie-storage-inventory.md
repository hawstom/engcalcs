...
| `ec_nolog` | `lib/config.inc.php` | 10 years | Stops a browser being counted **at all**. NOT redundant with `ec_consent` — refusing consent still writes an undeduplicated 'visit' row, while this writes nothing anywhere. Verified 2026-08-12 against all five log writers | **Exempt** — same reasoning; it only honours a choice |

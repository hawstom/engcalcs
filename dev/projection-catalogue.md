...
## 5. Recommendation — SHIPPED 2026-09-14, and the one thing that changed on the way

All five points below were built as written. `js/data/epsg-projected.json` holds all **5,346** live
projected CRS at **343 KB raw / 79 KB gzipped** (the §4 estimate said 336/79), generated by
`dev/scripts/generate_projection_catalogue.js`, checked offline by
`dev/scripts/projection_catalogue_check.php` in `check_all.sh`, fetched when the chooser first
opens, with the 183 built-in rows kept as the fallback and one IOGP credit line in the box.

**WHAT CHANGED: THE INPUT IS A PINNED PyPI WHEEL, NOT IOGP'S REST API — and the reason is
vendoring, not correctness.** `apps.epsg.org` is the first-party source and was measured and
rejected the same day. Its list endpoint returns an area of use as a NAME rather than a box, so
building this file from it costs ~5,346 CRS detail calls plus ~3,700 extent calls — and at the end
of nine thousand requests you hold a snapshot **nothing publishes a hash for**. PROJ's `proj.db` is
the same dataset, states the register version in its own `metadata` table, and arrives inside a
pyproj wheel whose sha256 **PyPI publishes** — so the input is pinned and a later run on another
machine can prove it used the same bytes. Counts reproduced §4 exactly (5,708 total, 5,346 live,
EPSG v12.029 of 2025-10-02, PROJ 9.8.1), which is the check on that substitution.

**AND THE DIGEST IS A WEAKER CLAIM HERE THAN FOR BOOTSTRAP, WHICH THE MANIFEST SAYS OUT LOUD.**
Bootstrap's digests match the SRI hashes Bootstrap publishes, so they prove the committed bytes ARE
the upstream release. Nobody publishes a hash of "every live projected CRS as an array of arrays",
because that derivation is ours: the digest proves only that the file has not changed since it was
committed. Re-running the generator against the pinned wheel is what proves it matches the
register. That gap is why the wheel is named by hash in `dev/vendor-manifest.json`, and why the
entry lives in a `data` block rather than under `packages` — an entry there is asked three
questions this one cannot answer (GPL-compatible licence, agreement with `package.json`,
byte-identity with a published release), and a check carrying three waivers is one nobody believes.

The original recommendation, kept because it is what was built:

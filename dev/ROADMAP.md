...
- 5|135| **Swahili native review — the illustrative low-resource-tier quality probe.** Tom has a
  ...
  **UPDATE 2026-07-22 — first attempt was AI-based, not a true native read.** Tom's contact returned a
  trap-term assessment sheet, but it was generated by running the terms through an AI model (AI
  checking AI — circular; not the human ground truth this probe exists for). Do NOT treat as
  native-verified: do not mass-apply its edits, do not change sw `0.65`. It contradicts our deliberate
  Task 128 decision to KEEP `kichwa` for "head" (AI wants `kimo cha maji`), and some advice cuts
  against localization (suggests retaining English "head"/"SG", even `hasara ya head`). Genuine value:
  converges with the earlier internal flag that velocity head uses `kimo` while gross/net/head use
  `kichwa` — so **"head" in Swahili is genuinely unsettled**; the crisp question for a real native
  engineer is "`kichwa cha maji` vs `kimo cha maji` for hydraulic head?". Also flag for a human:
  specific gravity `uzito maalum` (may read as specific *weight*), crown `taji` (royal crown, likely
  wrong for pipe crown). Still need an actual human read; the AI sheet is a flag, not a verdict.

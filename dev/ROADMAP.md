...
- 100|545| **[H] 64 English strings are waiting on Tom's wording, and they were recorded nowhere.**
  ...
  - **AND THE LIST IS A FILE NOW, NOT A COMMAND** (Tom, 2026-08-27: *"I don't have a list of
    strings to review. I don't know what to do with the php file you gave me. Please give me a
    file."*). Quite right, and it was the same promise in a different costume — a script he has to
    run is a list he does not have. **`dev/new-english-keys.md`** is generated by
    `new_english_keys.php --write`, tracked, grouped by feature, carries every string in full, and
    is kept current by a BLOCKING `--check` in `check_all.sh`. It cannot go stale between the day a
    key is written and the day he reads it, which is the whole failure this task came from.
  - **[H] What is left is the reading**, and it is a sitting rather than a trickle.

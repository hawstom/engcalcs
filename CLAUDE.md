...
| `lpn_` | Looped Pipe Network, Map Interface — shipped 2026-07-30, still moving. (**The "preview" label is retired**, Tom 2026-08-09 — do not reintroduce it in README, menus, or page copy.) Canvas/map-centric looped network solved by the global gradient algorithm (`js/lpn-solver.js`), with a map editor over it (`js/looped-network.js`). Sibling of `bpn_`, which is unaffected. **A CORE CALCULATOR since 2026-08-11 (Task 251), so it is in scope in all 26 languages and sprint 251 translated it there.** (It was core-four-only from 2026-08-08 to 2026-08-11; do not repeat that as its current state — it is the stalest-sounding sentence in this file and it has already misled a session.) Check `dev/scripts/translation_coverage.json` and `php dev/scripts/translation_completion_matrix.php` rather than this line: THE FILE is what the tools read, and a scope decision that lives only in prose is not in effect. Element types are junction, reservoir, **tank**, pipe, pump, **valve**, text: **no extended-period simulation** yet (ROADMAP Task 248, a GATE on the LibreEPANET.org launch, ships all three; tanks and valves both landed 2026-08-14 and are the only two things ever to leave `dev/looped-network-calculator-scope.md`'s "Cut, not deferred" list). **Valves are the one place the two engines genuinely differ, and the difference is deliberate:** a throttle valve (TCV) is a minor loss on a zero-length link and solves in either engine, while PRV/PSV/FCV switch their own state inside the iteration and solve through EPANET ONLY — we did not write a second implementation, because Task 313 measured EPANET at ~9x our own solver's speed. A network holding one is ROUTED to EPANET automatically and the status bar says so, without rewriting the user's stored `engine` setting (the setting is a preference; the routing is a fact about this network), and the native solver refuses such a network BY NAME if the engine cannot be reached. `EngCalcs.lpnValveIsNative` is the one place that line is drawn and `EngCalcs.lpnLinkK` the one place a TCV's loss is read from its SETTING rather than its `k`. A tank is a fixed head at its water surface, which is what EPANET itself solves at t = 0; `EngCalcs.lpnIsFixedHead` is the one place that equivalence is declared, and `dev/lpn-spike/tank-harness.js` is the only thing in the repo asserting that a tank diameter is in the LENGTH unit while a pipe diameter three sections away in the same file is in millimetres — a solve never reads it, so nothing else can see that mistake. `dev/lpn-spike/valve-harness.js` is its sibling for the valve SETTING, which is a pressure, a flow or a bare coefficient depending on the type: an active valve's setting is invisible to `validate_epanet.js` **structurally**, because those types never reach an engine comparison at all. **Reads EPANET `.inp` files** (`js/lpn-inp.js`, Task 196) — importing the supported subset and reporting every difference, never rejecting and never dropping in silence; it does not write one yet (Task 281). Scope: `dev/looped-network-calculator-scope.md`; ROADMAP Task 146 and its 146.nn children |
...
## Deploying: two facts that are not in any file you will be editing

Both were found inside closed roadmap tasks during the Task 320 archive move, where nobody re-scans.
They are here because each one bites at the worst possible moment and gives no other warning.
...
- **`Options -Indexes` in `.htaccess` needs `AllowOverride Options`, and where that grant is missing
  ...
  granted on the current host 2026-07-28 (Task 156). **If the site ever moves, test that line first
  and drop it if the new host 500s.** This is the single directive that can take the whole suite
  down on a host change, and it fails closed with no partial symptom to diagnose from.
- **`../sitemap.xml` is regenerated by `dev/scripts/generate_sitemap.php` but is NOT tracked by
  git.** Deployment here is `git pull`, so a regenerated sitemap does not travel with the commit —
  it must be re-uploaded to the site root, or deleted URLs stay advertised to search engines
  (Task 232). Any task that adds or removes a page owes that upload.

Related and easy to forget for the same reason: **`git pull` does not preserve mtimes**, so every
file's `filemtime` on production is its checkout time. That is why the service worker is generated
at request time rather than built (Task 318) — a baked file cannot know the mtimes the pages will
actually request.
